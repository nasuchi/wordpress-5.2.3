...
// Start class
if ( ! class_exists( 'TJ_Extras_About_Me_Widget' ) ) {
	class TJ_Extras_About_Me_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			$this->social_services_array = apply_filters( 'tj_about_me_widget_profiles', array(
				'facebook'		=> array(
					'name'		=> 'Facebook',
					'url'		=> ''
				),
			...
			parent::__construct(
				'tj_about_me',
				esc_html__( '&raquo; About Me', 'tj-extras' ),
				array(
					'classname'   => 'widget-tj-about-me about-me-widget',
					'description' => esc_html__( 'Adds a about me widget.', 'tj-extras' ),
		...
		public function social_widget_style() { ?>
			<style>
				.tj-about-me-widget-services-list { padding-top: 10px; }
				.tj-about-me-widget-services-list li { cursor: move; background: #fafafa; padding: 10px; border: 1px solid #e5e5e5; margin-bottom: 10px; }
				.tj-about-me-widget-services-list li p { margin: 0 }
				.tj-about-me-widget-services-list li label { margin-bottom: 3px; display: block; color: #222; }
				.tj-about-me-widget-services-list li label span.fa { margin-right: 10px }
				.tj-about-me-widget-services-list .placeholder { border: 1px dashed #e3e3e3 }
			...
			<script>
				( function( $ ){
					$(document).ajaxSuccess(function(e, xhr, settings) {
						var widget_id_base = 'tj_about_me';
					...
					function tjSortServices() {
						$( '.tj-about-me-widget-services-list' ).each( function() {
							var id = $(this).attr( 'id' );
							$( '#'+ id ).sortable( {
			...
			echo $args['before_widget'];
				...
				<div class="tj-about-me">
					...
					<div class="tj-about-me-avatar clr">
						...
						// Display the name
						if ( $name ) :?>
							<h3 class="tj-about-me-name"><?php echo esc_html( $name ); ?></h3>
						<?php endif; ?>
					...
					// Display the text
					if ( $text ) : ?>
						<div class="tj-about-me-text clr"><?php echo do_shortcode( $text ); ?></div>
					<?php endif;

					// Display the social
					if ( $social_services ) : ?>

						<ul class="tj-about-me-social style-<?php echo esc_attr( $social_style ); ?>">
							<?php
							// Original Array
		...
		public function form( $instance ) {
			$instance = wp_parse_args( ( array ) $instance, array(
				'title'       		=> esc_html__( 'About Me', 'tj-extras' ),
				'avatar' 			=> TE_URL . 'assets/img/about-avatar.png',
				'name'  			=> esc_html__( 'John Doe', 'tj-extras' ),
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'avatar' ) ); ?>"><?php esc_html_e( 'Image URL', 'tj-extras' ); ?>:</label> 
				...
				<input class="tj-widget-upload-button button button-secondary" type="button" value="<?php esc_html_e( 'Upload Image', 'tj-extras' ); ?>" />
			</p>

			<p>
			    <label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e('Name:', 'tj-extras') ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Description:','tj-extras' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('social_style') ); ?>"><?php esc_html_e('Social Style:', 'tj-extras'); ?></label>
				<br />
				<select class='widget-select widefat' name="<?php echo esc_attr( $this->get_field_name('social_style') ); ?>" id="<?php echo esc_attr( $this->get_field_id('social_style') ); ?>">
					<option value="color" <?php selected( $instance['social_style'], 'color' ) ?>><?php esc_html_e( 'Color', 'tj-extras' ); ?></option>
					<option value="light" <?php selected( $instance['social_style'], 'light' ) ?>><?php esc_html_e( 'Light', 'tj-extras' ); ?></option>
					<option value="dark" <?php selected( $instance['social_style'], 'dark' ) ?>><?php esc_html_e( 'Dark', 'tj-extras' ); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('target') ); ?>"><?php esc_html_e( 'Social Link Target:', 'tj-extras' ); ?></label>
				<br />
			...
			<ul id="<?php echo esc_attr( $field_id_services ); ?>" class="tj-about-me-widget-services-list">
			...
			<script type="text/javascript">
				(function($) {
					...
					$( document ).ready( function() {
						...
						$( '.tj-widget-upload-button' ).click(function(e) {
...
register_widget( 'TJ_Extras_About_Me_Widget' );